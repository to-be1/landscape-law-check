...
if auto_small_scale:
    st.sidebar.success(f"💡 **판별 결과: {detected_law}**\n\n특례법 대상 현장입니다. 조경 50% 완화 및 생태면적률 제외가 아래 시스템에 완전히 자동 적용되었습니다.")
else:
    st.sidebar.info(f"💡 **판별 결과: {detected_law}**\n\n일반 법규(조경 100% 의무, 생태면적률 권장)가 정상 적용됩니다.")
...
st.sidebar.checkbox("🏘️ 특례법 완화 (조경면적 1/2) 자동 적용됨", value=is_small_scale, disabled=True)
landscaping_area_plan = st.sidebar.number_input("🟩 계획 조경면적 (㎡)", min_value=0.0, value=750.0, step=50.0)
natural_ground_plan = st.sidebar.number_input("🟫 계획 자연지반면적 (㎡)", min_value=0.0, value=150.0, step=10.0)

st.sidebar.checkbox("🌱 생태면적률 의무 대상 자동 적용됨", value=is_eco_target, disabled=True)
eco_area_plan = st.sidebar.slider("🍃 계획 생태면적률 (%)", min_value=0.0, max_value=100.0, value=15.0, step=0.5)
open_space_plan = st.sidebar.number_input("⛲ 계획 공개공지면적 (㎡)", min_value=0.0, value=0.0, step=10.0)

st.sidebar.markdown("---")
st.sidebar.header("🌲 4. 식재 및 부대시설 수량 입력")
tree_count = st.sidebar.number_input("🌳 계획 교목 총 수량 (주)", min_value=0, value=100, step=10)
evergreen_tree_count = st.sidebar.number_input("🌲 계획 상록교목 수량 (주)", min_value=0, value=25, step=5)
special_tree_count = st.sidebar.number_input("✨ 계획 특성수 수량 (주)", min_value=0, value=15, step=5)

shrub_count = st.sidebar.number_input("🌿 계획 관목 총 수량 (주)", min_value=0, value=800, step=50)
evergreen_shrub_count = st.sidebar.number_input("🍃 계획 상록관목 수량 (주)", min_value=0, value=200, step=10)
...
law_list = [
    ...
    {"분류": "지자체 조례", "근거 법규 및 지침명": f"{local_gov} 건축조례 조경 의무 기준 조항", "시행/개정년도": f"{local_gov} 최신 조례"},
...
st.info(f"**📍 현장 주소:** {display_address}\n\n**🔍 자동 인식 지자체:** {local_gov} | {zone_type} | {building_type} | 대지면적: {site_area:,.1f}㎡")
st.write("")
...
if is_small_scale:
    ...
    landscape_metric_text = f"지자체 조례에 따른 대지면적의 {req_landscape_ratio*100}% 이상 확보"
