...
# [주소지 입력 추가] 보고서 및 파일명에 사용될 주소지
project_address = st.sidebar.text_input(
    "📝 프로젝트 주소지 (지번/도로명)", 
    value="충청남도 천안시 서북구 성성동 일원", 
    help="보고서 상단과 다운로드될 엑셀 파일명에 표기될 현장 주소를 입력하세요."
)
...
if project_category == "노후 건축물 정비 (재개발/재건축)":
    ...
    # 판별 로직: 대지면적 1만(가로구역 2만) 및 기존 세대수 기준
...
st.info(f"**📍 현장 주소:** {display_address}\n\n**🔍 검토 조건:** {region} | {zone_type} | {building_type} | 대지면적: {site_area:,.1f}㎡")
st.write("")
...
if is_small_scale:
    ...
    landscape_metric_text = f"선택 지자체 조례에 따른 대지면적의 {req_landscape_ratio*100}% 이상 확보"
...
# 파일명에 특수문자나 띄어쓰기가 들어갈 경우를 대비해 전처리
