...
project_address = st.sidebar.text_input("📝 프로젝트 주소지 (지번/도로명)", value="불광동 600", help="동 이름만 입력해도 주요 지자체는 자동 추론됩니다.")
...
# ---------------------------------------------------------
# 🧠 [핵심 보완] 스마트 주소 추론 및 파싱 엔진
# ---------------------------------------------------------
# 1차 정규식 파싱 ('시', '군', '구' 찾기)
match = re.search(r'([가-힣]+(시|군|구))', project_address)
local_gov_name = match.group(1) if match else ""

# 2차 스마트 추론 (시/군/구가 없을 때 동 이름으로 유추)
smart_inferred = False
if not local_gov_name:
    # 유명 동/지역명 매핑 DB (실무에서 자주 쓰이는 곳 위주로 계속 추가 가능)
    dong_mapping = {
        "서울": ["불광", "강남", "서초", "송파", "여의도", "종로", "명동", "은평", "마포", "성수"],
        "인천": ["구월", "송도", "청라", "부평"],
        "수원": ["인계", "광교", "영통"],
        "부산": ["해운대", "서면", "광안"],
        "부천": ["중동", "상동"],
    ...
    for city, dongs in dong_mapping.items():
        if any(dong in project_address for dong in dongs):
            local_gov_name = f"{city}특별시" if city == "서울" else f"{city}광역시" if city in ["인천", "부산"] else f"{city}시"
            smart_inferred = True
            break
...
if not local_gov_name:
    ...
    st.sidebar.warning("⚠️ 주소에 '시/군/구'가 명확하지 않아 전국 기본 조례가 적용됩니다. (예: 서울특별시 은평구 불광동)")
elif smart_inferred:
