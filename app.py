...
if is_small_scale:
    ...
    eco_req_str = "해당 없음"
    eco_plan_str = f"{eco_area_plan:.1f} %"
    eco_pass = "N/A"
else:
...
# 주민공동시설 총량제 판별 로직
if b_type != "공동주택 (아파트)":
    comm_total_text, comm_req_str, comm_plan_str, comm_pass = "해당 없음", "해당 없음", "해당 없음", "N/A"
    senior_text, senior_req_str, senior_plan_str, senior_pass = "해당 없음", "해당 없음", "해당 없음", "N/A"
    play_text, play_req_str, play_plan_str, play_pass = "해당 없음", "해당 없음", "해당 없음", "N/A"
    daycare_text, daycare_req_str, daycare_plan_str, daycare_pass = "해당 없음", "해당 없음", "해당 없음", "N/A"
    sports_text, sports_req_str, sports_plan_str, sports_pass = "해당 없음", "해당 없음", "해당 없음", "N/A"
    library_text, library_req_str, library_plan_str, library_pass = "해당 없음", "해당 없음", "해당 없음", "N/A"
else:
    # 1. 총량제
    ...
    if household_count < 150: 
        senior_text, senior_req_str, senior_plan_str, senior_pass = "설치 의무 없음", "해당 없음", f"{senior_area_plan:,.1f} ㎡", "N/A"
    else:
        senior_req = 50 + (household_count * 0.1)
        senior_text = f"의무대상 권장면적 (50㎡+세대수×0.1㎡)"
        senior_req_str = f"{senior_req:,.1f} ㎡"
        senior_plan_str = f"{senior_area_plan:,.1f} ㎡"
        senior_pass = senior_area_plan >= senior_req

    # 3. 어린이놀이터
    if household_count < 150: 
        play_text, play_req_str, play_plan_str, play_pass = "설치 의무 없음", "해당 없음", f"{play_area_plan:,.1f} ㎡", "N/A"
    elif household_count < 300:
        play_text = "의무대상 (적정면적 설치 권장)"
        play_req_str = "적정 면적 확보"
        play_plan_str = f"{play_area_plan:,.1f} ㎡"
        play_pass = play_area_plan > 0
    elif household_count < 1000:
        play_req = 200 + (household_count * 1.0)
        play_text = f"의무대상 권장면적 (200㎡+세대수×1㎡)"
        play_req_str = f"{play_req:,.1f} ㎡"
        play_plan_str = f"{play_area_plan:,.1f} ㎡"
        play_pass = play_area_plan >= play_req
    else:
        play_req = 500 + (household_count * 0.7)
        play_text = f"의무대상 권장면적 (500㎡+세대수×0.7㎡)"
        play_req_str = f"{play_req:,.1f} ㎡"
        play_plan_str = f"{play_area_plan:,.1f} ㎡"
        play_pass = play_area_plan >= play_req
    ...
    if household_count < 300: 
        daycare_text, daycare_req_str, daycare_plan_str, daycare_pass = "설치 의무 없음", "해당 없음", f"{daycare_area_plan:,.1f} ㎡", "N/A"
    else:
        daycare_text = "의무대상 (보육정원별 인가면적 적용)"
        daycare_req_str = "인가 기준 확인 요망"
        daycare_plan_str = f"{daycare_area_plan:,.1f} ㎡"
        daycare_pass = daycare_area_plan > 0

    # 5. 주민운동시설
    if household_count < 500: 
        sports_text, sports_req_str, sports_plan_str, sports_pass = "설치 의무 없음", "해당 없음", f"{sports_area_plan:,.1f} ㎡", "N/A"
    else:
        sports_text = "의무대상 (종목별 경기장 규격 권장)"
        sports_req_str = "총량 내 적정 확보"
        sports_plan_str = f"{sports_area_plan:,.1f} ㎡"
        sports_pass = sports_area_plan > 0

    # 6. 작은도서관
    if household_count < 500: 
        library_text, library_req_str, library_plan_str, library_pass = "설치 의무 없음", "해당 없음", f"{library_area_plan:,.1f} ㎡", "N/A"
    else:
        library_text = "의무대상 (문체부 100㎡ 내외 권장)"
        library_req_str = "100.0 ㎡ 이상"
        library_plan_str = f"{library_area_plan:,.1f} ㎡"
...
def print_law_row(category, title, legal_text, law_source, req_str, plan_str, is_pass):
    ...
    with c3:
        ...
        <div style='background-color:#f8f9fa; padding:10px; border-radius:5px; border-left: 4px solid #2E5A44;'>
            ...
            <div style='font-size:0.95em; color:#0056b3;'>🎯 <b>계획:</b> <b>{plan_str}</b></div>
        </div>
        """, unsafe_allow_html=True)
    with c4:
        if is_pass == "N/A" or "해당 없음" in req_str:
            st.info("➖ 제외")
            csv_status = "제외"
        elif is_pass:
            st.success("✅ 적합")
    ...
    report_data.append({"검토 항목": title, "법적 기준": legal_text, "법정 요구치": req_str, "내 계획 수치": plan_str, "결과": csv_status})
...
if b_type == "공동주택 (아파트)":
    ...
    print_law_row("3. 주민시설", " ├ 어린이놀이터", play_text, law_guideline, play_req_str, play_plan_str, play_pass)
    print_law_row("3. 주민시설", " ├ 어린이집", daycare_text, law_guideline, daycare_req_str, daycare_plan_str, daycare_pass)
    print_law_row("3. 주민시설", " ├ 주민운동시설", sports_text, law_guideline, sports_req_str, sports_plan_str, sports_pass)
    print_law_row("3. 주민시설", " └ 작은도서관", library_text, law_guideline, library_req_str, library_plan_str, library_pass)

print_law_row("4. 기타", "자전거 보관소", "법정 주차대수의 20% 이상", law_bike, f"{legal_bike_parking:,.0f} 대", f"{bike_parking_plan:,.0f} 대", bike_parking_plan >= legal_bike_parking)
print_law_row("4. 기타", "식재 토심", "아파트 교목 0.9m (인공 1.2m 권장)", law_soil_depth, "설계 도서 내 토심 명기", "권장 사양 확인 완료", True)
